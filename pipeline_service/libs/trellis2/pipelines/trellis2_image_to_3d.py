...
class Trellis2ImageTo3DPipeline(Pipeline):
    ...
    def decode_latent(
        ...
        """
        if shape_slat.shape[0] != tex_slat.shape[0]:
            raise ValueError(
                f"Batch size mismatch between shape_slat and tex_slat: "
                f"{shape_slat.shape[0]} != {tex_slat.shape[0]}"
            )
        ...
        for sample_idx in range(shape_slat.shape[0]):
            ...
            tex_slat_i = tex_slat[sample_idx]
            meshes, subs = self.decode_shape_slat(shape_slat_i, resolution)
            tex_voxels = self.decode_tex_slat(tex_slat_i, subs)
