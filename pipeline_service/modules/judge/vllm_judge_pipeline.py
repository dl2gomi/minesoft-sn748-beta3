...
from .judge_pipeline import JudgePipeline, JudgeResponse
...
from schemas.bytes import image_to_base64
        ...
        messages = [
            {"role": "system", "content": SYSTEM_PROMPT},
            {
                ...
                "content": [
                    ...
                    {
                        "type": "image_url",
                        "image_url": {"url": f"data:image/png;base64,{image_to_base64(image_tensor_to_pil(prompt_image))}"},
                    },
                    {"type": "text", "text": "First 3D model (4 different views):"},
                    {
                        "type": "image_url",
                        "image_url": {"url": f"data:image/png;base64,{image_to_base64(image_tensor_to_pil(img1_grid))}"},
                    },
                    {"type": "text", "text": "Second 3D model (4 different views):"},
                    {
                        "type": "image_url",
                        "image_url": {"url": f"data:image/png;base64,{image_to_base64(image_tensor_to_pil(img2_grid))}"},
                    },
